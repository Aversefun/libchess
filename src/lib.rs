...
impl TryFrom<(Option<u8>, Option<u8>)> for Position {
    ...
    fn try_from(value: (Option<u8>, Option<u8>)) -> Result<Self, Self::Error> {
        ...
        Ok(Self {
            packed_pos: ((value.1.unwrap_or(0) & 0b111) << 3)
                | (value.0.unwrap_or(0) & 0b111)
                | if value.1.is_some() { 0b0100_0000 } else { 0 }
                | if value.0.is_some() { 0b1000_0000 } else { 0 },
        })
    }
}
...
impl TryFrom<(u8, u8)> for Position {
    ...
    fn try_from(value: (u8, u8)) -> Result<Self, Self::Error> {
        ...
        Ok(Self {
            packed_pos: ((value.1 & 0b111) << 3) | (value.0 & 0b111) | 0b1100_0000,
        })
    }
}
...
impl From<Position> for (Option<u8>, Option<u8>) {
    fn from(value: Position) -> Self {
        ...
        (
            if value.has_file() { Some(file) } else { None },
            if value.has_rank() { Some(rank) } else { None },
        )
    }
}
...
impl TryFrom<&str> for Position {
    ...
    fn try_from(value: &str) -> Result<Self, Self::Error> {
            ...
            value.chars().count() == 2,
            "&str should be exactly character length 2"
        );
...
impl Move {
    ...
    pub fn parse_alg_char_slice(
        ...
        let (piece, color) = if castling {
            ...
            // assume that it is the piece
            let ch = s[0];
            s = &s[1..];
            Piece::from_piece_char_strict(ch).unwrap()
        } else {
            // likely pawn; could be a position or something like ca1 or 1a2 or something
            (Piece::Pawn, None)
        };
        ...
        let from = if ((s.len() == 3 && !character_is_capturing(s[0]))
            || (character_is_capturing(s[1]) && s.len() >= 4))
            && !castling
        {
            let ch = s[0];
            s = &s[1..];
            if ch.is_ascii_digit() && ch > '0' {
                ...
                // A file
                Some(Position::try_from((
                    Some(ch.to_ascii_lowercase() as u32 as u8 - b'a'),
                    None,
                ))?)
            } else {
            ...
            s = &s[2..];
                ...
                ch[0].is_ascii_alphabetic(),
                "bad from file (not ascii alphabetic)"
            );
        ...
        let promotion = if could_promote {
            match s.len() {
                ...
                3 => Some(
                    Piece::from_piece_char(s[2])
                        .ok_or(anyhow!("invalid promotion piece"))?
                        .0,
                ),
                4.. => {
                    // probably fourth symbol - any extra characters are ignored
                    Some(
                        Piece::from_piece_char(s[3])
                            .ok_or(anyhow!("invalid promotion piece"))?
                            .0,
                    )
                }
